...
<!-- Use this as the "How-To" that operators will follow on the live system ????? -->
...
3. Click the upload area and select the 'CA Signed Certificate' file from local folder in `.cer` or `.pem` format. <!-- Explain this a bit or cross-refer -->
...
* If Root/Intermediate CA <!-- Certificate --> is missing, the system will reject the upload. Therefore ensure that the CA certificates are uploaded first. [Certificate Trust Store](partner-administration#certificate-trust-store)
...
**Success check:** Approved partner-policy links will appear in the partner's policy list. <!-- Refine a bit -->
