...
This section serves as a manual for users who need to register and configure a Credential\
Partner account. It covers the sequential actions required within the PMS portal---from initial\
registration to the final submission of a policy request---to enable the secure issuance of identity\
credentials.

### **2. What is a Credential Partner?**

A Credential Partner is an entity authorized to generate and issue physical or digital identity\
documents (like ID cards) to citizens. In the MOSIP ecosystem, once an identity is verified, the\
Credential Partner receives the authorized data packets to produce the final credential.
...
Onboarding is performed by the Partner and Partner Admin using the PMS portal.\
Before you begin, ensure you have your organization's digital certificates and have confirmed\
that the Partner Admini has uploaded the necessary Trust Chains.

#### **Onboarding Steps - Partner**

4. **Self-Registration by Partner:** Navigate to the PMS portal,complete the registration form for your organization, and log in to your account.
5. **Policy Group Selection by Partner:** Select the appropriate "Credential Partner" policy group to unlock your specific dashboard view.
6. **CA Certificate Upload by Partner:** Upload your organization's CA certificate.

**Note**: This requires that the Partner Admin has already uploaded the Trust Chain (Root and Intermediate certificates).

7. **Policy Request by Partner:** Initiate a policy request which involves three sub-steps:
   3. Select the required Policy.
   4. Map the Biometric Extractor Provider Configuration.
   5. Select the desired Credential Type.
8. **Policy request Submitted:** Your request is sent to the Partner Admin for review and approval.
9. **Partner Admin Approval:** Once partner admin approves the policy request the partner onboarding is complete.

#### **Onboarding Steps - Partner Admin**:

10. **Upload Certificate Trust Chain:** Partner admin uploads the certificate trust chain (Root, Intermediate and leaf certificates).
11. **Approve Policy Request:** Partner admin will approve or reject the policy request Submitted by Credential Partner.
12. **Create Biometric Extractor Provider Configuration:** Partner admin should create the required biometric extractor provider configuration before the partner create a policy request.
...
13. The Credential Partner can register themselves on MOSIP PMS portal by clicking **Register** on the Login Page, a form comes up.
...
22. Credential partner log in to the PMS portal and lands on the Dashboard.
